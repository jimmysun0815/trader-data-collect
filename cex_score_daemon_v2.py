...
from cex_scorer_v3 import (
    AdaptiveScoreNormalizer,
    DEFAULT_ZEFF_MODEL_PATH,
    SignalOptimizer,
    _needs_warmup,
    _normalize_ts,
    _warmup_normalizer_recursive,
    score_cex as score_cex_v3,
)
...
def main() -> int:
    ...
    use_zeff_model = getattr(args, "use_zeff_model", False)
    zeff_model_path = (args.zeff_model_path or DEFAULT_ZEFF_MODEL_PATH or "").strip()
    if use_zeff_model and not zeff_model_path:
        print("[cex-score] use_zeff_model=True but no path, disabling MLP zeff", flush=True)
    ...
    print(
        ...
        + (f" use_zeff_model=True path={zeff_model_path}" if use_zeff_model else ""),
        flush=True,
    )
            ...
            if use_zeff_model:
                try:
                    res = score_cex_v3(
                        ...
                        lookback_seconds=int(args.lookback_s),
                        normalizer_cache_dir=cache_dir,
                        symbol=symbol,
                        return_meta=True,
                        use_zeff_model=True,
                    ...
                    if binance_ok and extra_factor is not None and is_normalized:
                        z_eff = float(z_score) * float(extra_factor)
                    else:
                        z_eff = None
            elif is_normalized and binance_ok and extra_factor is not None:
                z_eff = float(z_score) * float(extra_factor)
