...
_ZEFF_MLP_FEATURES = ["zscore", "raw_score", "delta_15s_pct", "btc_vol_60s", "raw_score_ema", "hour_of_day"]
...
def _build_zeff_features(
    ...
    """
    构建 MLP zeff 的 6 维特征；live 容错：btc_mid 用 last_btc_mid 或默认，特征缺失填 0。
    """
    # btc_mid fallback: 上一次缓存的 btc_mid 或默认 50000.0（避免 0 导致 cum_change/vol 异常）
    if not mid_series:
        if cache_key not in _LOGGED_MID_SERIES_EMPTY:
            print(
                "[cex] mid_series empty, btc_mid fallback to last known or default",
                flush=True,
            )
            _LOGGED_MID_SERIES_EMPTY.add(cache_key)
        btc_mid = _LAST_BTC_MID.get(cache_key) or 50000.0
    else:
        btc_mid = mid_series[-1][1]
        _LAST_BTC_MID[cache_key] = btc_mid

    # 注意：训练时 delta_15s_pct 是未来收益，live 用过去 15s 作为代理，可能有分布偏移
    delta_15s_pct = _compute_delta_pct_live(mid_series, window_sec=15.0)

    # 特征缺失时用 0 补全（如 btc_vol_60s 不足 60s）
    if vol_60s is None:
        vol_60s = 0.0
    try:
        hour_of_day = datetime.fromtimestamp(float(timestamp), tz=timezone.utc).hour
    except Exception:
        hour_of_day = 0
    ...
    return {
        ...
        "delta_15s_pct": float(delta_15s_pct),
